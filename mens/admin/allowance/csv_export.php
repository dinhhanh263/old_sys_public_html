...
// データの取得----------------------------------------------------------------------
$dSql = "SELECT s.*,p.base_salary,p.allowance FROM " . $table . " as s,posi_salary as p WHERE s.del_flg = 0 and s.type=p.position and s.type not in(19,21) ".$dWhere." order by s.type,s.id";
...
//カウンセリング手当、施術手当対象
$obj_c = array("8","10","11","15","17","18","22","30");
$obj_t = array("8","10","11","15","17","18","22","30");
...
if ( $dRtn3->num_rows >= 1 ) {
	echo mb_convert_encoding("ｽﾀｯﾌ名,出勤形態,前月基本給,当月基本給,役職名,役職手当,基本給+役職手当,ｶｳﾝｾﾘﾝｸﾞ手当,施術手当,研修手当,出張手当,ｽﾀｯﾌ紹介,社長賞,目標達成手当,社販分,手当合計,総計\n","SJIS-win", "UTF-8");
	$i = 0;
	$cnt = array();
	while ( $data = $dRtn3->fetch_assoc() ) {
		...
		if($allowance['coun_allowance']){
			...
			$counseling_query = $GLOBALS['mysqldb']->query( $counseling_sql );
		...
		}elseif(in_array($type, $obj_t) && $shop_id<999 ){

			$treatment_sql =  "select count(s.id) as cnt from sales as s,course as c2 where s.del_flg=0 and s.type=2 and s.r_times<>0 and s.course_id=c2.id and c2.type=0 and s.staff_id=".$data['id']." and substring(s.pay_date,1,7)='".str_replace("/","-",$ym2)."' ";
			$treatment_query = $GLOBALS['mysqldb']->query( $treatment_sql );
			while ( $result = $treatment_query->fetch_assoc() ) {
