...
function csv_export () {
    ...
    img.onload = function(){
	    if( img.height > 0){ // 読み込みに失敗すれば 0 になる。
		    document.search.action = "prduct_csv.php";
...
<div id="content">
	...
	<div id="page-heading">
		...
		<?php
			$gProductNo = array( 1 =>"モイスチャーローション", 2=>"プリモ");
			$gProducStatus = array( 1 => "物販", 2=>"プレゼント", 5=>"社販",10=>"店舗使用");
		?>
		<h1>
			物販管理一覧
			...
			<?php } ?>
				...
				<input type="submit" value=" 表示 "  style="height:25px;" onClick="form.action='product_stock.php';return true" />
			</span>
		...
		<p><a href="../help/register_manual/index.php#3" target="_blank">使い方ヘルプ</a></p>
	</div>
	<!-- end page-heading -->
	<p>※濃い色の行は無料プレゼント</p>
...
if ( $dRtn3 ) {
	...
	while ( $data = $dRtn3->fetch_assoc() ) {
		...
		echo 	'<td><a href="../sales/register.php?id='.$data['sales_id'].'&customer_id='.$data['customer_id'].'" target="_blank">'.$data['sales_id'].'</a></td>'; //売上ID（清算ID）
		...
		echo 	'<td>'.$data['no'].'</td>';
		echo 	'<td><a href="../sales/register.php?id='.$data['sales_id'].'&customer_id='.$data['customer_id'].'" target="_blank">'.($data['name'] ? $data['name'] : $data['name_kana']).'</a></td>';
