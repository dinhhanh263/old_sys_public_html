...
<!--
<link rel="stylesheet" type="text/css" href="../js/jquery-ui-multiselect-widget-master/jquery.multiselect.css" />
<link rel="stylesheet" type="text/css" href="../js/jquery-ui-multiselect-widget-master/demos/assets/style.css" />
<link rel="stylesheet" type="text/css" href="../js/jquery-ui-multiselect-widget-master/demos/assets/prettify.css" />
<script type="text/javascript" src="../js/jquery-ui-1.11.2.custom/jquery-ui.min.js"></script>
<script type="text/javascript" src="../js/jquery-ui-multiselect-widget-master/src/jquery.multiselect.js"></script>
...
<div id="content">
	...
	<div id="page-heading">
		...
		<h1>
			...
			<?php } ?>
				店舗：<select name="shop_id" style="height:25px;width:150px;" <?php echo $authority_shop['id'] ? "disabled" : "" ?> id="sample" ><?php Reset_Select_Key( $shop_list , $authority_shop['id'] ? $authority_shop['id'] : $_POST['shop_id'] );?></select>
				<!--店舗：<select name="shop_id"  <?php echo $authority_shop['id'] ? "disabled" : "" ?> size='3' multiple id="sample" ><?php Reset_Select_Key( $shop_list , $authority_shop['id'] ? $authority_shop['id'] : $_POST['shop_id'] );?></select>
-->
...
<?php
	...
	if(!$_POST['customer_id'] && !$_POST['keyword'])	{
		echo '<tr><td colspan="14"></td><td>内訳</td>';
		//echo '<tr><td colspan="14" class="priceFormat">月額:</td><td class="priceFormat">'.number_format($cnt_monthly).' 名</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">契約者数:</td><td class="priceFormat">'.number_format($cnt_pack).' 名</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">未契約者数:</td><td class="priceFormat">'.number_format($dGet_Cnt4).' 名</td></tr>';
		...
		//echo '<tr><td colspan="14" class="priceFormat">来店率:</td><td class="priceFormat">'.$percent_came.'</td></tr>';
		//echo '<tr><td colspan="14" class="priceFormat">成約率:</td><td class="priceFormat">'.$percent_contract.'</td></tr>';

	if(isset($total['4'])){
		echo '<tr><td colspan="14" class="priceFormat">クーリングオフ:</td><td class="priceFormat">'.number_format($cnt4).' 名</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">クーリングオフ返金:</td><td class="priceFormat">'.number_format($total['4']).' 円</td></tr>';
	}
	if(isset($total['5'])){
		echo '<tr><td colspan="14" class="priceFormat">中途解約:</td><td class="priceFormat">'.number_format($cnt5).' 名</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">中途解約返金:</td><td class="priceFormat">'.number_format($total['5']).' 円</td></tr>';
	}
	// if(isset($total['20'])){
	// 	echo '<tr><td colspan="14" class="priceFormat">月額退会:</td><td class="priceFormat">'.number_format($cnt20).' 名</td></tr>';
	// 	echo '<tr><td colspan="14" class="priceFormat">月額退会返金:</td><td class="priceFormat">'.number_format($total['20']).' 円</td></tr>';
	// }
	if(isset($total['6'])){
		echo '<tr><td colspan="14" class="priceFormat">プラン変更:</td><td class="priceFormat">'.number_format($cnt6).' 名</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">プラン変更差額:</td><td class="priceFormat">'.number_format($total['6']).' 円</td></tr>';
	}
	/*if(isset($total['7'])){
		echo '<tr><td colspan="14" class="priceFormat">売掛回収:</td><td class="priceFormat">'.number_format($cnt7).' 名</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">売掛回収金額:</td><td class="priceFormat">'.number_format($total['7']).' 円</td></tr>';
	}*/
	if(isset($cnt12)){
		echo '<tr><td colspan="14" class="priceFormat">自動解約:</td><td class="priceFormat">'.number_format($cnt12).' 名</td></tr>';
	}

		echo '<tr><td colspan="14" class="priceFormat">現金売上:</td><td class="priceFormat">'.number_format($total_cash).' 円</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">カード売上:</td><td class="priceFormat">'.number_format($total_card).' 円</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">銀行振込:</td><td class="priceFormat">'.number_format($total_transfer).' 円</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">ローン売上:</td><td class="priceFormat">'.number_format($total_loan).' 円</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">売掛回収:</td><td class="priceFormat">'.number_format($total_payment).' 円</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">売掛金:</td><td class="priceFormat">'.number_format($total_balance).' 円</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">売掛含む総合計:</td><td class="priceFormat">'.number_format($total_balance + $total_without_balance).' 円</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">売掛含まない総合計:</td><td class="priceFormat">'.number_format($total_without_balance).' 円</td></tr>';
		if(($cnt_monthly+$cnt_pack+$dGet_Cnt4)) {
			echo '<tr><td colspan="14" class="priceFormat">客単価((売掛含む総合計 - 物販売上)/来店数):</td><td class="priceFormat">'.number_format(($total_balance + $total_without_balance - $total_cnt51)/($cnt_monthly+$cnt_pack+$dGet_Cnt4)).' 円</td></tr>';
		}else{
			echo '<tr><td colspan="14" class="priceFormat">客単価((売掛含む総合計 - 物販売上)/来店数):</td><td class="priceFormat">'.number_format(0).' 円</td></tr>';
		}
		//echo '<tr><td colspan="14" class="priceFormat">キャンペーン売上:</td><td class="priceFormat">'.number_format($total_campaign).' 円</td></tr>';
		echo '<tr class="hr"><td colspan="14" class="priceFormat">物販利用者:</td><td class="priceFormat">'.number_format($cnt_product).' 人</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">物販売上:</td><td class="priceFormat">'.number_format($total_cnt51).' 円</td></tr>';
		if($cnt_product){
			echo '<tr><td colspan="14" class="priceFormat">物販客単価（物販売上/物販利用者）:</td><td class="priceFormat">'.number_format($total_cnt51/$cnt_product).' 円</td></tr>';
		}else{
			echo '<tr><td colspan="14" class="priceFormat">物販客単価（物販売上/物販利用者）:</td><td class="priceFormat">'.number_format(0).' 円</td></tr>';
		}
		// echo '<tr><td colspan="14" class="priceFormat">物販出庫数:</td><td class="priceFormat">'.number_format($cnt51).' 個</td></tr>';
		echo '<tr><td colspan="14" class="priceFormat">（物販無料プレゼント人数）:</td><td class="priceFormat">('.number_format($cnt51_noprice).' 人)</td></tr>';
	}
}
?>
