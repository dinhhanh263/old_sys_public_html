<?php include_once("../library/customer/card_ng.php");?>
	...
	<div id="page-heading">
		<h1>
			...
			<span style="margin-left:20px;">
				<a href="./card_ng.php?contract_date=<?php echo $pre_date?>&shop_id=<?php echo $_POST['shop_id'];?>"><img src="../images/table/paging_left.gif" title="前日" /></a>
				...
				<a href="./card_ng.php?contract_date=<?php echo $next_date?>&shop_id=<?php echo $_POST['shop_id'];?>"><img src="../images/table/paging_right.gif" title="翌日" /></a>
				<select name="search_shop_id" style="height:25px;width:120px;" ><?php Reset_Select_Key( $shop_list , $_POST['search_shop_id']);?></select>
				<select name="debit_type" style="height:25px;width:120px;" ><?php Reset_Select_Key( $gDebitType , $_POST['debit_type']);?></select>
				<select name="card_ng" style="height:25px;width:120px;" ><?php Reset_Select_Key( $gCardNG2 , $_POST['card_ng']);?></select>
	...
	<tr>
		...
		<div id="content-table-inner">
			...
			<div id="table-content">
				...
				<tr>
					<th class="table-header-repeat line-left minwidth-1"><a href=""><font size="-2">カードタイプ</font></a>	</th>
...
if ( $dRtn3 ) {
	...
	while ( $data = $dRtn3->fetch_assoc() ) {
		...
		if($data['digicat_ng_flg']){ 
			$card_type 		.= $gDebitType['1'];
			$ng_pattern 	.= $gCardNG[$data['digicat_ng_flg']];
			echo '<input name="digicat_ng_flg" type="hidden" value="'.$data['digicat_ng_flg'].'">';
		}
		if($data['nextpay_end_ng_flg']){
			$card_type 		.= $gDebitType['2'];
			$ng_pattern 	.= $gCardNG[$data['nextpay_end_ng_flg']];
			echo '<input name="nextpay_end_ng_flg" type="hidden" value="'.$data['nextpay_end_ng_flg'].'">';
		} 
		if($data['nextpay_op_ng_flg']){
			$card_type 		.= $gDebitType['3'];
			$ng_pattern 	.= $gCardNG[$data['nextpay_op_ng_flg']];
			echo '<input name="nextpay_op_ng_flg" type="hidden" value="'.$data['nextpay_op_ng_flg'].'">';
		} 
		
		echo '<tr'. ( $i%2==0 ? ' class="alternate-row"' : '' ) .'>';
		echo 	'<td>'.$card_type.'</td>';
		echo 	'<td>'.$ng_pattern.'</td>';
		echo 	'<td>'.$data['no'].'</td>';
		...
		echo 	'<td><a href="javascript:document.forms[\'frm'.$i.'\'].submit();" onclick="return confirm(\''.$data['no'].'を引落可に変更しますか？\')" title="OK" >OK</a></td>';
		echo '</tr>';
		...
		echo '<input name="debit_type" type="hidden" value="'.$_POST['debit_type'].'">';
		echo '<input name="card_ng" type="hidden" value="'.$_POST['card_ng'].'">';
