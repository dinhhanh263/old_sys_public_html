...
if ( $dRtn3->num_rows >= 1 ) {
	...
	while ( $data = $dRtn3->fetch_assoc() ) {
		...
		echo 	'<td>'.$data['no'].'</td>';
		echo 	'<td title="'.$data['name'].'"><a  rel="facebox" href="mini.php?id='.$data['id'].'">'.($data['name_kana'] ? $data['name_kana'] : $data['name']).'</a></td>';
	...
	if($authority_level<=6 || $authority['id']==106 && $_POST['ctype']<2){
		...
		echo 		'<a href="../reservation/edit.php?id='.$rsv['id'].'&shop_id='.$rsv['shop_id'].'&hope_date='.$rsv['hope_date'].'&from_cc=1" title="予約詳細" class="icon-1 info-tooltip"></a>';
