    ...
    <div id="content-table-inner">
      ...
      <div id="table-content">
        <?php if($gMsg) echo $gMsg;?>
          <table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table" class="adfrm">
            <tr>
              <th class="table-header-repeat"><a class="th_title" href="">ID</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">広告コード</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">媒体(ブログ)名</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">種類</a></th>
              <th class="table-header-repeat line-left" ><a class="th_title" href="">サイト名</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">LP名</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">集客/求人</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">グループ</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">請求媒体</a></th>
              <!--<th class="table-header-repeat line-left" ><a href="">単価</a></th>-->
              <th class="table-header-repeat line-left"><a class="th_title" href="">チラシ番号</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">店舗表示文章</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">発行日</a></th>
              <th class="table-header-repeat line-left"><a class="th_title" href="">代理店</a></th>
              <th class="table-header-repeat line-left minwidth-1" colspan="2"><a class="th_title" href="">オプション</a></th>
            </tr>
            <tr>
             ...
             　  <input name="daili_id" type="hidden" value="<?php echo($_POST['daili_id']); ?>">
                 <td>新規</td>
                 <td><input class="inp-form2" type="text" name="adcode" value="<?php echo($_POST['id'] ? "" : $_POST['adcode']); ?>"/></td>
                 <td><input class="inp-form" type="text" name="name"  value="<?php echo($_POST['id'] ? "" : $_POST['name']); ?>"/></td>
                 ...
                 <td><input class="inp-form2" type="text" name="flyer_no" value="<?php echo($_POST['id'] ? "" : $_POST['flyer_no']); ?>" /></td>
                 <td><input class="inp-form" type="text" name="memo" value="<?php echo($_POST['id'] ? "" : $_POST['memo']); ?>" /></td>
                 <td><input class="inp-form2" type="text" name="release_date" value="<?php echo date('Y/n/j'); ?>"/></td>
                 <td><select style="height:30px;width:150px;" name="agent_id"><option value=""></option><?php Reset_Select_Key( $gAgent ,$_POST['id'] ? "" : $_POST['agent_id'] ) ?></select></td>
            ...
            ?>
                ...
                <td rowspan="2"><?php echo(  $data['id']  ); ?></td>
                <td><input class="inp-form2" type="text" name="adcode" value="<?php echo(  $data['adcode']  ); ?>" /></td>
                ...
                <td><select style="height:30px;width:150px;" name="request_id"><?php Reset_Select_Array_Group(getDatalistArray2("m_ad","ad_group") , $data['request_id'],$gADGroup);?></select></td>
                 ...
                 <td><input class="inp-form2" type="text" name="flyer_no" value="<?php echo( View_Cook( $data['flyer_no'] ) ); ?>" /></td>
                 <td><input class="inp-form" type="text" name="memo" value="<?php echo( View_Cook( $data['memo'] ) ); ?>" /></td>
    		        <td><input class="inp-form2" type="text" name="release_date" value="<?php echo( View_Cook( $data['release_date'] ) ); ?>" readonly/></td>
    		        <td><select style="height:30px;width:150px;" name="agent_id"><option value=""></option><?php Reset_Select_Key( $gAgent , View_Cook( $data['agent_id'] )) ?></select></td>
    		        <td><a href="javascript:document.forms['adfrm<?php echo $i;?>'].submit();" onclick="return confirm('<?php echo(  $data['adcode'].".". $data['name'] ); ?>を変更しますか？')" title="変更" class="icon-1 info-tooltip"></a></td>
  		        </form>
  		        <form action="<?php echo($_SERVER['PHP_SELF']); ?>" method="post" name="frm<?php echo($data['id']); ?>">
  		           <input name="action" type="hidden" value="delete">
  				       ...
  				       <input name="keyword" type="hidden" value="<?php echo($_POST['keyword']); ?>">
  				       <input name="daili_id" type="hidden" value="<?php echo($_POST['daili_id']); ?>">
  				       <td><a href="javascript:document.forms['frm<?php echo($data['id']); ?>'].submit();" onclick="return deleteChk('<?php echo(  $data['adcode'].".". $data['name'] ); ?>を削除しますか？')" title="削除" class="icon-2 info-tooltip"></a></td>
  			      </form>
        		</tr>
            <tr>
              <?php
                if($data['type']==4){
                  ...
                  $ad_url = "https://menskireimo.jp/lp/".$data['adcode'];
                }else{
                  $ad_url = "https://menskireimo.jp/".($data['page_name'] ? $data['page_name']."/" : "").( ($data['type']==3 || $data['id']==916) ? "" : "?adcode=".$data['adcode']);
                }
              ?>
              <td colspan="5">
                  広告コード付きURL:
              ...
              <td colspan="8">
                <button class="adcode_btn" id="copy-dynamic<?php echo $data['id'];?>" >URLコピー</button>
                  <script type="text/javascript">
                    $(function(){
                ...
                <a class="inlilne" href ="<?php echo $ad_url ;?>" target="_blank">
                  <button class="adcode_btn">プレビュー </button>
                </a>
              </td>
            </tr>
             <?php
