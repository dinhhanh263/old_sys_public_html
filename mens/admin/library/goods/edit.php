...
if( $_POST['action'] == "edit" ) {
	if(!Get_Table_Row("item_brand"," WHERE name = '".addslashes($_POST['goods_name'])."'")){
		$GLOBALS['mysqldb']->query("insert  item_brand set name = '".addslashes($_POST['goods_name'])."'");
	}
	if(!Get_Table_Row("item_kinsei"," WHERE name = '".addslashes($_POST['kinsei'])."'")){
		$GLOBALS['mysqldb']->query("insert  item_kinsei set name = '".addslashes($_POST['kinsei'])."'");
	}
...
if($auth_sql) {
	...
	$GLOBALS['mysqldb']->query($auth_sql);
}
...
//品名リスト作成
$item_brand = $GLOBALS['mysqldb']->query( "SELECT * FROM item_brand order by name" );
...
//金製リスト作成
$item_kinsei = $GLOBALS['mysqldb']->query( "SELECT * FROM item_kinsei order by name" );
...
//品名リスト作成
$item_landing = $GLOBALS['mysqldb']->query( "SELECT * FROM item_landing order by name" );
