...
if( $_POST['action'] == "edit" ) {
	...
	// 	$gMsg = "<span class='error_msg'>※個数を入力してください。</span>";
		...
		// 売上tableに反映------------------------------------------------------------------------
		// $sales_field  = array("id","type","customer_id","shop_id","staff_id","rstaff_id","option_name","option_price","option_card","product_no","product_count","pay_date","reg_date","edit_date","del_flg");
		// $sales_field2  = array("id","type","customer_id","shop_id","staff_id","rstaff_id","option_name","option_price","option_card","product_no","product_count","pay_date","edit_date","del_flg");
		...
		// 更新処理
			...
			foreach ($product_no as $key) {
				...
				if($result_no !== false){ //更新 = 選択商品noと登録済商品noが一致(=既存IDがある)したら
					...
					$_POST['del_flg'] = 0;
					$_POST['id'] = Input_New_Data($product_stock,$product_stock_field);
				}else if($_POST['product_count'] == 0){ //新規 = 既存IDがなく、かつ個数が0ならcontinue;
					continue;
				}else{
...
if($_POST['id']){ //物販レジ（登録済）を開いた時
	...
	$dSql = "SELECT count(*) FROM ".$table. " s WHERE type=51 and del_flg = 0";
		...
		$i = 0;
		while ( $prduct_array = $dRtn3->fetch_assoc() ) {
...
if($product_stocks){
	...
	// $pdf_param ="?shop_name=".$shop['name']."&shop_zip=".$shop['zip']."&shop_pref=".$gPref[$shop['pref']]."&shop_address1=".$shop_address1."&shop_address2=".$shop_address2."&shop_tel=".$shop['tel']."&name=".($customer['name'] ?$customer['name'] : $customer['name_kana'] )."&product_name=".$product_name[$data['product_no']]."&tax=".$tax."&tax2=".$tax2."&fixed_price=".(1800 * $tax2 * $data['product_count'])."&discount=".""."&price=".($data['option_price']+$data['option_card'])."&payment=".($data['option_price']+$data['option_card'])."&option_name=".""."&option_price=".""."&balance=".""."&pay_date=".$data['reg_date'];
}
