...
//担当リスト------------------------------------------------------------------------
$tantou_sql = $GLOBALS['mysqldb']->query( "select * from tantou WHERE status<>1 order by id" );
while ( $result = $tantou_sql->fetch_assoc() ) {
  $tantou_list[$result['id']] = $result['name'];
}
	
$customer_sql = $GLOBALS['mysqldb']->query( "select * from customer WHERE lat>0 and lng>0 order by lat,lng" );
