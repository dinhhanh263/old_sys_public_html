...
$from = "FROM contract_P AS tp,customer AS c,";
...
$from.= " WHERE c.id=tp.customer_id AND c.id=r.customer_id AND tp.del_flg =0 AND c.del_flg =0 AND tp.status =0 AND tp.loan_status>1 AND payment_loan>0 ";
...
$dSql = "SELECT tp.*,c.no as no,c.name as name,c.name_kana as name_kana,c.tel as tel,c.reg_date as reg_date ".$from.$dWhere."order by tp.contract_date LIMIT ".$dStart.",".$dLine_Max;
