...
if($_POST['action'] == "new" || $_POST['action'] == "update" && $_POST['id']){
	...
	//	$gMsg .= "<font color='red' size='-1'>※請求媒体を選択してください。</font><br>";
		...
		if($_POST['page_name'] && !Get_Table_Row("item_landing"," WHERE name = '".addslashes($_POST['page_name'])."'")){
			$GLOBALS['mysqldb']->query("insert  item_landing set name = '".addslashes($_POST['page_name'])."'");
		}
...
//ランディングリスト作成
$item_landing = $GLOBALS['mysqldb']->query( "SELECT * FROM item_landing where del_flg=0 order by ldp_name" );
