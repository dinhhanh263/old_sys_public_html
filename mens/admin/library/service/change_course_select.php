...
if( $_GET['customer_id'] != "" )  {
	// すべての契約コース情報
	$all_contract = Get_Table_Array($c_table,"*"," WHERE del_flg=0 and customer_id = '".addslashes($_GET['customer_id'])."'");
	...
	$reservation = Get_Table_Row("reservation"," WHERE del_flg=0 and id = '".addslashes($_GET['reservation_id'])."'");

	// 契約IDごとのレジ情報
	foreach ($all_contract as $key => $value) {
		if($value['sales_id'] != 0 ) $sales[$value['id']] = Get_Table_Row("sales"," WHERE del_flg=0 and id = '".addslashes($value['sales_id'])."'");
	}
}
