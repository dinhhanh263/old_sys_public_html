		...
		*/
		...
		foreach($lines as $val){
			...
			if( $result==0){
				...
				if( $customer = Get_Table_Row("customer"," WHERE del_flg=0 and no like '%".$no."%' and (replace(name, '　', '') like '%".$name1."%' or replace(name_kana, '　', '') like '%".$name1."%' )") ){
					$result_sql .= ",customer_id='".$customer['id']."'";

					// 条件：月額契約中である
					if( $contract = Get_Table_Row("contract"," WHERE del_flg=0 and customer_id = '".$customer['id']."' and course_id in(".$month_id." ) order by id desc limit 1") ){
				...
				if( $customer = Get_Table_Row("customer"," WHERE del_flg=0 and no like '%".$no."%' and (replace(name, '　', '') like '%".$name1."%' or replace(name_kana, '　', '') like '%".$name1."%' )") ){
					...
					// 条件：月額契約中である
					if( $contract = Get_Table_Row("contract"," WHERE del_flg=0 and customer_id = '".$customer['id']."' and course_id in(1,5,15,16,18,19,44,45,46,47,48,49,50,51,55,56,63,64,70 ) order by id desc limit 1") ){
						$result_sql .= ",contract_id='".$contract['id']."'";
