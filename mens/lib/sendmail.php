...
if( $count > 0 ){
	foreach ( $mailmg_data as $key=>$val ) {
		...
		while ( $data = $dRtn->fetch_assoc() ) {
			...
			if( $data['format'] && sendMailHtml( $data['mail'], $val['subject'], $body, $val['sender'] , $cc="", $bcc=$_POST['sender'] , MAIL_RETURN ) ) {
				$send_flg = true; $cnt +=1;
			}
			elseif( sendMail( $data['mail'], $val['subject'], $body, $val['sender'] , $cc="", $bcc=$_POST['sender'] , MAIL_RETURN ) ) {
				$send_flg = true; $cnt +=1;
			}
			sleep(10);//10秒待ち
		}
