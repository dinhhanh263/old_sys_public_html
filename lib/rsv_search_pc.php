...
function getTimeListByShop($table,$shop_list,$tennpo,$nittei,$gTime2,$length,$gParty,$gClosed,$gPartyTimeList1,$gPartyTimeList2,$gEndingShop,$gEndingDate,$gEndingTime){
            ...
            $rsv_list[] = array(
              'html' => '<table id="tbl02" border="0" cellspacing="0.5" cellpadding="0"><tr>
                 <td class="date"><p class="'.getDateClass($value1).'">'.getDateYobi2($value1).'</p></td>
                     ...
                     <td><p><a href="#step3" onclick="return result(\''.$shop_list[$value].'\',\''.$value1.'\',\''.$value2.'\',\''.$value.'\',\''.$length.'\');" class="yoyakubi"><img src="../img/counseling/btn_select_on.gif" width="100" height="30" alt="選択"></a></p></td>
                 </tr>'
            );
            // 別の日程に指定
            $is_nittei = true;
 ...
 $rsv_list[] = array('html' => '</table>' );
  ...
  if(!$is_nittei && ($_GET['tenpo1'] || $_GET['tenpo2'] || $_GET['tenpo3'] || $_GET['tenpo4'] || $_GET['tenpo5'] ||
    ...
    $rsv_list = array();
    $rsv_list[] = array(
              'html' => '<p class="search_error error">別の日程をご選択ください。<br><a href="#step1"><img class="wink" src="../img/counseling/btn_search03.gif" width="362" height="63" alt=""></a></p>'
              );
