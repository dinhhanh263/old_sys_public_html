...
$table = "shop";
//店舗list
$sql = $pdo->query( "select * from shop WHERE del_flg = 0  AND status=2 and id<>6 order by id" );
if($sql){
...
$sql = 'SELECT * FROM job WHERE del_flg=0 AND session_id = ?';
$pdo->prepare($sql);
$pdo->bindParam(session_id());
$pdo->execute();
$result = $pdo->fetch();

if($mode && !strstr($_POST['comment'],"http") && !empty($result)){
...
//	$sql .= $sql_data.$mo_agent.",'".$_SESSION['AD_CODE']."','".$mobile_id."','".session_id()."','".$_SERVER['PHP_SELF']."','".$_SESSION['KIREIMO_REFERER']."','".$ua."',2,'".date("Y-m-d H:i:s"). "')";

//	$rtn = mysql_query( $sql );
    $sql = "INSERT INTO job ( mo_agent, adcode, mo_id, session_id, url, referer_url, user_agent, reg_flg, reg_date ) ";
    $sql .= ' VALUES (?, ?, ?, ?, ?, ?, ?, 2, ?);';
    $pdo->prepare($sql);
    $pdo->bindParam([
        $sql_data . $mo_agent,
