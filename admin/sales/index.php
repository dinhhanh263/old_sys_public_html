		...
		<div id="content-table-inner">
			...
			<div id="table-content">
				...
				<tr>
					...
					<th class="table-header-repeat line-left minwidth-1"><a href=""><font size="-2">社員番号</font></a></th>
					<th class="table-header-repeat line-left minwidth-1"><a href=""><font size="-2">担当者名</font></a></th>
...
if ( $dRtn3 ) {
	...
	while ( $data = $dRtn3->fetch_assoc() ) {
		//役務消化除外
		if($data['r_times'] && !$data['payment_cash'] && !$data['payment_card'] && !$data['payment_transfer'] && !$data['payment_loan'] && !$data['payment_coupon'] && !$data['option_price'] && !$data['option_transfer'] && !$data['option_card'] ) continue;
		...
		echo 	'<td><font color="red">'.($course_type[$data['course_id']] ? $gResType6[$data['type']] : ($data['type']==6 && $contract['conversion_flg'] ? "プラン組替" : $gResType3[$data['type']])).'</font></td>';
		}
		echo 	'<td>'.$shop_list[$data['shop_id']].'</td>';
		echo 	'<td>'.$data['pay_date'].'</td>';
		echo 	'<td>'.$data['no'].'</td>';
		echo 	'<td>'.($data['name'] ? $data['name'] : $data['name_kana']).'</td>';
		echo 	'<td>'.( $data['type']==51 ? "-" : $course_list[$data['course_id']]).'</td>';
		...
		echo 	'<td>'.( $data['staff_id']==0 ? "-" : $staff['code'] ).'</td>';
		echo 	'<td>'.( $data['staff_id']==0 ? "-" : $staff['name'] ).'</td>';
		...
		echo '<tr><td colspan="17"></td><td>内訳</td>';
		echo '<tr><td colspan="17" class="priceFormat">月額:</td><td class="priceFormat">'.number_format($cnt_monthly).' 名</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">パック:</td><td class="priceFormat">'.number_format($cnt_pack).' 名</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">未契約:</td><td class="priceFormat">'.number_format($dGet_Cnt4).' 名</td></tr>';
		...
		// echo '<tr><td colspan="17" class="priceFormat">来店率:</td><td class="priceFormat">'.$percent_came.'</td></tr>';
		// echo '<tr><td colspan="17" class="priceFormat">成約率:</td><td class="priceFormat">'.$percent_contract.'</td></tr>';

	if(isset($total['4'])){
		echo '<tr><td colspan="17" class="priceFormat">クーリングオフ:</td><td class="priceFormat">'.number_format($cnt4).' 名</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">クーリングオフ返金:</td><td class="priceFormat">'.number_format($total['4']).' 円</td></tr>';
	}
	if(isset($total['5'])){
		echo '<tr><td colspan="17" class="priceFormat">中途解約:</td><td class="priceFormat">'.number_format($cnt5).' 名</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">中途解約返金:</td><td class="priceFormat">'.number_format($total['5']).' 円</td></tr>';
	}
	if(isset($total['20'])){
		echo '<tr><td colspan="17" class="priceFormat">月額退会:</td><td class="priceFormat">'.number_format($cnt20).' 名</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">月額退会返金:</td><td class="priceFormat">'.number_format($total['20']).' 円</td></tr>';
	}
	if(isset($total['6'])){
		echo '<tr><td colspan="17" class="priceFormat">プラン変更:</td><td class="priceFormat">'.number_format($cnt6).' 名</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">プラン変更差額:</td><td class="priceFormat">'.number_format($total['6']).' 円</td></tr>';
	}
	/*if(isset($total['7'])){
		echo '<tr><td colspan="17" class="priceFormat">売掛回収:</td><td class="priceFormat">'.number_format($cnt7).' 名</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">売掛回収金額:</td><td class="priceFormat">'.number_format($total['7']).' 円</td></tr>';
	}*/
	if(isset($cnt12)){
		echo '<tr><td colspan="17" class="priceFormat">自動解約:</td><td class="priceFormat">'.number_format($cnt12).' 名</td></tr>';
	}

		echo '<tr><td colspan="17" class="priceFormat">現金売上:</td><td class="priceFormat">'.number_format($total_cash).' 円</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">カード売上:</td><td class="priceFormat">'.number_format($total_card).' 円</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">銀行振込:</td><td class="priceFormat">'.number_format($total_transfer).' 円</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">ローン売上:</td><td class="priceFormat">'.number_format($total_loan).' 円</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">売掛回収:</td><td class="priceFormat">'.number_format($total_payment).' 円</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">売掛金:</td><td class="priceFormat">'.number_format($total_balance).' 円</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">売掛含む総合計:</td><td class="priceFormat">'.number_format($total_balance + $total_without_balance).' 円</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">売掛含まない総合計:</td><td class="priceFormat">'.number_format($total_without_balance).' 円</td></tr>';
		if(($cnt_monthly+$cnt_pack+$dGet_Cnt4)){
			echo '<tr><td colspan="17" class="priceFormat">客単価((売掛含む総合計 - 物販売上)/来店数):</td><td class="priceFormat">'.number_format(($total_balance + $total_without_balance - $total_cnt51)/($cnt_monthly+$cnt_pack+$dGet_Cnt4)).' 円</td></tr>';
		}else{
			echo '<tr><td colspan="17" class="priceFormat">客単価((売掛含む総合計 - 物販売上)/来店数):</td><td class="priceFormat">'.number_format(0).' 円</td></tr>';
		}
		echo '<tr class="line"></tr><tr><td colspan="17" class="priceFormat">物販利用者:</td><td class="priceFormat">'.number_format($cnt_product).' 人</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">物販売上:</td><td class="priceFormat">'.number_format($total_cnt51).' 円</td></tr>';
		if(($cnt_product)){
			echo '<tr><td colspan="17" class="priceFormat">物販客単価（物販売上/物販利用者）:</td><td class="priceFormat">'.number_format($total_cnt51/$cnt_product).' 円</td></tr>';
		}else{
			echo '<tr><td colspan="17" class="priceFormat">物販客単価（物販売上/物販利用者）:</td><td class="priceFormat">'.number_format(0).' 円</td></tr>';
		}
		// echo '<tr><td colspan="17" class="priceFormat">物販出庫数:</td><td class="priceFormat">'.number_format($cnt51).' 個</td></tr>';
		echo '<tr><td colspan="17" class="priceFormat">（物販無料プレゼント人数）:</td><td class="priceFormat">('.number_format($cnt51_noprice).' 人)</td></tr>';
}
?>
