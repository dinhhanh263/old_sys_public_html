...
elseif(!$_POST['new_flg'] && !$data['new_flg']) $contract = Get_Table_Row("contract"," WHERE customer_id <>0 and customer_id = '".addslashes($data['customer_id'] ? $data['customer_id'] : $customer['id'])."' and del_flg=0 and (status=0 or status=5 or status=6 or status=7) ".$where_contract." order by status in (0,7) desc, contract_date desc, id desc"); // 契約中コース指定status=0,5
