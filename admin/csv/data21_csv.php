...
SELECT t.status,t2.contract_date '継続契約日', h.name '店舗', t.customer_id '顧客ID', c.no '会員番号', c.name '名前', u.name '旧コース名', u2.name '新コース名',
 (t2.fixed_price - t2.discount) '新契約金額', t.r_times '旧コース消化済回数',
  ...
  (SELECT name FROM staff s2 WHERE s2.id = t2.staff_id) '新担当'
...
LEFT JOIN course u2 ON u2.id = t2.course_id, shop h, course u
...
if ( $dRtn3->num_rows >= 1 ) {
	echo mb_convert_encoding("継続契約日,店舗,顧客ID,会員番号,名前,旧コース名,新コース名,新契約金額,旧コース消化済回数,旧担当,新担当\n","SJIS-win", "UTF-8");
