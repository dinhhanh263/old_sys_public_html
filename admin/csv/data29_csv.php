...
$dSql = 'SELECT
	...
	END AS "契約ステータス"
FROM contract WHERE del_flg = 0 AND status IN (0,3,4) AND conversion_flg = 0 AND (SELECT type FROM course WHERE id = course_id) = 0 AND (SELECT treatment_type FROM course WHERE id = course_id) = 0 AND (SELECT ctype FROM customer WHERE id = customer_id) = 1 AND SUBSTRING(pay_complete_date, 1, 7 ) = "'.$date1.'" AND (old_course_id > 0 AND old_contract_id IN (SELECT id FROM contract WHERE del_flg = 0 AND status = 4 AND conversion_flg = 0)) OR id IN (SELECT id FROM contract WHERE del_flg = 0 AND status = 0 AND conversion_flg = 0 AND (SELECT type FROM course WHERE id = course_id) = 0 AND (SELECT treatment_type FROM course WHERE id = course_id) = 0 AND (SELECT ctype FROM customer WHERE id = customer_id) = 1 AND SUBSTRING(pay_complete_date, 1, 7 ) = "'.$date1.'" AND customer_id IN (SELECT customer_id FROM contract WHERE del_flg = 0 AND status = 4 AND conversion_flg = 0 AND id IN (SELECT old_contract_id FROM contract WHERE id IN (SELECT loan_cancel_before_contract_id FROM contract WHERE del_flg = 0 AND status = 0))))';
...
if ( $dRtn3->num_rows >= 1 ) {
  echo mb_convert_encoding("会員番号,会員氏名,従業員番号,従業員氏名,契約コース名,契約日,支払完了日,請求金額,売掛金,契約ステータス\n","SJIS-win", "UTF-8");
