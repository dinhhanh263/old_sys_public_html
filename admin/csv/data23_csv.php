...
// データの取得----------------------------------------------------------------------
$dSql = "
SELECT t.status,t2.contract_date '継続契約日', h.name '店舗', t.customer_id '顧客ID', c.no '会員番号', c.name '名前', u.name '旧コース名', u2.name '新コース名',
...
INNER JOIN contract t2 ON t2.del_flg =0 AND t2.customer_id = t.customer_id AND t2.id <> t.id AND t2.status=0 AND t2.times>=6 AND t2.contract_date>t.contract_date
LEFT JOIN course u2 ON u2.id = t2.course_id, shop h, course u,sales s
...
AND u2.type=0
AND t.times>=6
AND t.times=t.r_times
AND s.del_flg=0
AND s.customer_id=c.id
AND s.reg_date=t2.reg_date
AND t2.contract_date BETWEEN '".$date1."' AND '".$date2."'
ORDER BY t2.contract_date,c.id
";
...
if ( $dRtn3->num_rows >= 1 ) {
	echo mb_convert_encoding("継続契約日,店舗,顧客ID,会員番号,名前,旧コース名,新コース名,新契約金額,旧コース消化済回数,旧担当,新担当,支払金額\n","SJIS-win", "UTF-8");

	while ( $data = $dRtn3->fetch_assoc() ) {

		echo mb_convert_encoding($data['継続契約日'],"SJIS-win", "UTF-8")  . ",";
		...
		echo mb_convert_encoding($data['支払金額'],"SJIS-win", "UTF-8"). ",";
		echo "\n";
	}
