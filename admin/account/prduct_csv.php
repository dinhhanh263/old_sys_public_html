...
// データの取得----------------------------------------------------------------------
$dSql = "SELECT ps.shop_id as sales_shop, p.name as product_name, sum(ps.product_count) as sum_count, ps.price,(ps.price*sum(ps.product_count)) as sum_price , ps.use_status, ps.pay_date, st.name, st.shop_id ";
$dSql .= "FROM " . $table . " as ps, sales as s, staff as st, product as p ";
$dSql .= "WHERE ps.del_flg = 0 AND s.del_flg = 0 AND st.del_flg = 0 AND p.del_flg = 0 AND st.status = 2 AND st.id=ps.staff_id AND ps.sales_id = s.id AND ps.product_no = p.id ".$dWhere;
$dSql .= "GROUP BY ps.shop_id, st.shop_id, ps.pay_date, ps.staff_id, ps.product_no, ps.use_status ORDER BY ps.pay_date,ps.reg_date ";
$dRtn3 = $GLOBALS['mysqldb']->query( $dSql ) or die('query error'.$GLOBALS['mysqldb']->error);
// //csv export----------------------------------------------------------------------

$filename = "staff_sales.csv";
header("Content-type:text/csv");
header("Content-Disposition:attachment;filename=".$filename);

if ( $dRtn3->num_rows >= 1 ) {
  echo mb_convert_encoding("販売用途,支払日,販売店舗,商品,定価,販売個数,販売金額,スタッフ,所属店舗\n","SJIS-win", "UTF-8");
