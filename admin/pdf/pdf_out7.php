...
$pdf->SetXY( 110, 127 );
$pdf->Cell(20, 5, mb_convert_encoding( "  保証期間", "SJIS", "UTF-8" ), 1,0,"C");
...
$pdf->Text( 15 , 75  , mb_convert_encoding( "1.キレイモ全身脱毛プラン・平日とく得プランについて", "SJIS", "UTF-8" ) );
...
　に限り、残回数相当分の無償役務を提供させていただきますので、付与された保証延長期間内に
　残回数分の施術を受けてくださいますようお願いいたします。
...
　いただきます。
　※例外的な場合については「5.施術に関して (補足)」をご覧ください。	
・シェービングのサービスは基本的に行っておりませんが、お客様自身で手の届きにくい 背中、うなじ、
　Oライン、ヒップについてはこちらでシェービングのお手伝いをさせていただきます。その他の箇所は
　ご予約の前日にお客様自身でシェービングを行っていただくようお願いいたします。
　※剃り残しのある箇所は当日の施術をお断りさせていただきますのでご了承ください。

【お支払いについて】
・契約日に契約代金のうち、一部を手付金としてお支払いいただきます。残金は契約日から30日以内にお支払
　いをお願いいたします。(初回の施術はお支払い後となります。)
　※30日以内に残金をお支払頂けない場合は、お電話等にてご連絡させていただく場合がございます。	
　※契約日から90日以内に残金をお支払いただけない場合は、契約の意思がなく、また、手付金を放棄したも
　のとして契約を解除させていただきますのでご注意ください。
　※また、一度こちらの解約が成立した場合、お客様がお支払いされた手付金を充当して元のプランに復帰
　するなどの対応は一切いたしかねますのでご注意ください。
・代金の支払い方法は、現金払い、クレジットカード払い、当社指定金融機関へのお振込のほか、当社指定の
　ローン会社とローン契約をしていただくことによる立替払いをご選択いただけます。また、複数の決済
　手段を用いてお支払いいただくことも可能です。
...
$pdf->SetFont( KOZMIN,'U' , 12 );
$pdf->Text( 15 , 20, mb_convert_encoding( "2. スペシャルプランについて", "SJIS", "UTF-8" ) );

$pdf->SetXY( 17, 20);
$pdf->SetFont( KOZMIN,'' , 10 );
$pdf->MultiCell(170, 6, mb_convert_encoding( "
・スペシャルプランをご希望のお客様につきましては、別紙「スペシャルプランに関する同意書」をよく
　ご確認いただき、記載内容に同意をいただいた上でお申し込みください（ご署名が必要となります）。
", "SJIS", "UTF-8" ) , 0, 'L', 0);

$pdf->SetXY( 17, 20 );
$pdf->SetFont( KOZMIN,'U' , 12 );
$pdf->Text( 15 , 50 , mb_convert_encoding( "3. 平日とく得プランについて", "SJIS", "UTF-8" ) );

$pdf->SetXY( 17, 50);
$pdf->SetFont( KOZMIN,'' , 10 );
$pdf->MultiCell(170, 6, mb_convert_encoding( "
・平日とく得プランとは、予約の取得可能日や時間帯が限定される代わりに、全身脱毛プランよりお得な価格で提供させていただくコースです。
・平日とく得プランでの予約取得可否については、以下をご確認ください。
", "SJIS", "UTF-8" ) , 0, 'L', 0);

$pdf->Text( 17 , 80  , mb_convert_encoding( "予約ができる時間帯", "SJIS", "UTF-8" ) );
$pdf->SetXY( 17, 82);
$pdf->Cell(140, 5, mb_convert_encoding( "平日の12:30〜14:00・14:00〜15:30・16:30〜18:00 の時間帯", "SJIS", "UTF-8" ), 1,0,"L");
$pdf->Text( 17 , 95  , mb_convert_encoding( "予約ができない時間帯（制限時間帯）", "SJIS", "UTF-8" ) );
$pdf->SetXY( 17, 97);
$pdf->Cell(140, 5, mb_convert_encoding( "土曜日・日曜日の全時間帯", "SJIS", "UTF-8" ), 1,0,"L");
$pdf->SetXY( 17, 102);
$pdf->Cell(140, 5, mb_convert_encoding( "祝日法に定める、国民の休日・振替休日・国民の祝日の全時間帯", "SJIS", "UTF-8" ), 1,0,"L");
$pdf->SetXY( 17, 107);
$pdf->Cell(140, 5, mb_convert_encoding( "上記を除く平日の11:00〜12:30・18:00〜19:30・19:30〜21:00 の時間帯", "SJIS", "UTF-8" ), 1,0,"L");

$pdf->SetXY( 17, 112 );
$pdf->MultiCell(170, 5, mb_convert_encoding( "
・会員様専用ページからの予約の取得に際しては、上記の制限時間帯を選択することができなくなります。
・また、原則として店舗やお電話であっても制限時間帯のご予約はお断りさせていただきますので、予めご了承ください。
", "SJIS", "UTF-8" ) , 0, 'L', 0);

$pdf->SetFont( KOZMIN,'U' , 12 );
$pdf->Text( 15 , 140 , mb_convert_encoding( "4.会員様専用ページについて", "SJIS", "UTF-8" ) );
$pdf->SetXY( 17, 140 );
$pdf->SetFont( KOZMIN,'' , 10 );
$pdf->MultiCell(170, 5, mb_convert_encoding( "
...
$pdf->SetFont( KOZMIN,'U' , 12 );
$pdf->Text( 15 , 195  , mb_convert_encoding( "5.施術に関して (補足)", "SJIS", "UTF-8" ) );
...
　日までに書面の提出をいただけない場合、施術の提供は致しかねますのでご了承下さい。
・また、初回施術日以降も引き続き承諾書面の提出をいただけない場合、エステティックサービス契約を
　無効とさせていただくことがございます。
...
$pdf->SetFont( KOZMIN,'U' , 12 );
$pdf->Text( 15 , 20 , mb_convert_encoding( "6.安全に施術を受けていただくために", "SJIS", "UTF-8" ) );
...
$pdf->SetFont( KOZMIN,'U' , 12 );
$pdf->Text( 15 , 45 , mb_convert_encoding( "7.その他", "SJIS", "UTF-8" ) );
...
・ご契約プランの変更をご希望される場合には当社所定の手続が必要となります。ご契約プランの種別やお客
　様の状況によりご希望に添えない場合がございますので、別紙「コース変更等に関するご案内」をよく
　お読みいただいた上でお申込みください。
...
$pdf->SetFont( KOZMIN,'' , 9 );
$pdf->Text( 18 , 115  , mb_convert_encoding( substr($contract['contract_date'], 0,4)." 年 ".substr($contract['contract_date'], 5,2)." 月 ".substr($contract['contract_date'], 8,2)." 日", "SJIS", "UTF-8" ) );
...
", "SJIS", "UTF-8" ) );
$pdf->Text( 18 , 177  , mb_convert_encoding( substr($contract['contract_date'], 0,4)." 年 ".substr($contract['contract_date'], 5,2)." 月 ".substr($contract['contract_date'], 8,2)." 日", "SJIS", "UTF-8" ) );
