...
・シェービングのサービスは基本的に行っておりません。ご予約の前日にお客様自身でシェービングを行なって
　いだだくようお願いいたします。本サービスの提供に必要と判断した場合には、施術当日にシェービングのお手
　伝いをさせていただく場合もございますが、時間内に施術ができなかった箇所については消化扱いとなりますの
　でご了承ください。なお、お客様の要望により、シェービングのお手伝いをさせていただいた場合、シェービ
　ングによる事故に基づくお客様の損害は、当社スタッフに故意・重過失がない限り、当社及びスタッフは免責さ
　れるものとします。
", "SJIS", "UTF-8" ) , 0, 'L', 0);
...
$pdf->Rect(15 ,175 ,0 ,5); // 左から数えて縦線
$pdf->Rect(40 ,171 ,70 ,7);// 左から数えて上の囲み1
$pdf->Rect(40 ,175 ,0 ,5); // 左から数えて縦線2

$pdf->Rect(110 ,171 ,70 ,7);// 左から数えて上の囲み2
$pdf->Rect(110 ,175 ,0 ,5); // 左から数えて縦線3

$pdf->Rect(180 ,171 ,20 ,0);// 左から数えて上の囲み3
$pdf->Rect(180 ,175 ,0 ,5); // 左から数えて縦線4

$pdf->Rect(15 ,178 ,185 ,0); //基準となる横線


$pdf->SetFont( KOZMIN,'' , 10 );

$pdf->Text( 70 , 170  , mb_convert_encoding( "2ヶ月", "SJIS", "UTF-8" ) );
$pdf->Text( 140 , 170  , mb_convert_encoding( "2ヶ月", "SJIS", "UTF-8" ) );

$pdf->Text( 130 , 176  , mb_convert_encoding( "※契約期間更新", "SJIS", "UTF-8" ) );

$pdf->Text( 13 , 184  , mb_convert_encoding( "契約日", "SJIS", "UTF-8" ) );
$pdf->Text( 33 , 170  , mb_convert_encoding( "翌月1日", "SJIS", "UTF-8" ) );
$pdf->Text( 103 , 184  , mb_convert_encoding( "1.契約終了日", "SJIS", "UTF-8" ) );
$pdf->Text( 173 , 184  , mb_convert_encoding( "2.契約終了日", "SJIS", "UTF-8" ) );
...
$pdf->SetXY( 15, 17 );
$pdf->MultiCell(180, 6, mb_convert_encoding( "　
・書面による親権者様の承諾が無い未成年者のお客様については、施術の提供をお断りさせていただいておりま
　す。ご契約時に承諾書面をご用意できない場合、必ず初回施術日までに当社へご提出ください。初回施術日まで
　に書面の提出をいただけない場合、施術の提供はいたしかねますのでご了承ください。また、書面のご提出を
　いただくまで、マイページからの予約取得機能のご利用ができなくなります。
・また、初回施術日以降も引き続き承諾書面の提出をいただけない場合、エステティックサービス契約を無効と
　させていただくことがございます。
・ご予約当日、台風・大雪・地震など天変地異や著しい公共交通機関の遅延など特別な事情がある場合のキャン
　セルについては、対応を考慮させていただきます。ただし、当社の指定する期間内にご来店いただけなかった
　場合、当該期間内の施術は行われたものとさせていただきますので、指定期間内での早期のご予約を推奨いた
　します。
・災害等を原因とする臨時の休業等につきましては、原則として公式ホームページやマイページ等でお知らせい
　たしますので、そちらをご確認ください。なお、当社からのお知らせ等をご確認頂けなかったことによるお客
　様の損失等については責任を負いかねます。
・お客様都合による休会は承っておりませんので、あらかじめご了承ください。
・また、災害等（自然災害・疫病等の長期蔓延など）の場合であっても、当社は休会・返金の対応を原則として
　お客様の申告・申請を根拠として行います。当社より事前に一律の対応をお知らせする場合を除き、必ず当社
　までお申し出くださいますようお願いいたします。
", "SJIS", "UTF-8" ) , 0, 'L', 0);

$pdf->SetFont( KOZMIN,'U' , 10 );
$pdf->Text( 15 , 130  , mb_convert_encoding( "5．安全に施術を受けていただくために", "SJIS", "UTF-8" ) );
