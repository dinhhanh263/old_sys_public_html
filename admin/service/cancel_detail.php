...
function keisan2(){
	// 手数料
	var charge0 = Number(document.form1.charge.value); 
	var charge2 = Number(document.form1.charge2.value); 
	var charge3 = Number(document.form1.charge3.value); 
	var charge_total = (charge0 + charge2 + charge3);
	var charge = String( charge_total ).replace( /(\d)(?=(\d\d\d)+(?!\d))/g, '$1,' ); // 1,234
	...
	//返金額(手数料含)
	var payment0 = charge0 + charge2 + charge3 - remained_price0 - payment_loan;
					...
					<div id="content-table-inner">
							...
							<tr valign="top">
								...
								<form action="" method="post" name="form1" enctype="multipart/form-data" onSubmit="return conf1('');">
									...
									<input type="hidden" name="contract_id" value="<?php echo $data["id"];?>" />
									...
									<td>
										<table border="0" cellpadding="0" cellspacing="0"  id="id-form">
													...
													<div style="float:left;padding:40px;">													
														<table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table">
															...
															<tr>
																...
																<td class="regTotalPrice">￥<?php echo number_format(0-$remained_price);?></td>
																							</tr>
															<tr>
																<td class="reg">手数料</td>
																<td id="charge" class="regPrice">￥<?php echo number_format($charge+ $sales['charge2']+ $sales['charge3']);?></td>
															</tr>
															<tr>
																<td class="regTotal">返金額(手数料含)</td>
																<td id="payment" class="regTotalPrice">￥<?php echo $sales['id'] ? number_format($sales['payment_cash']+$sales['payment_card']+$sales['payment_transfer']+$sales['charge']+$sales['charge2']+$sales['charge3']) : number_format($charge+$charge2+$charge3-$remained_price) ;?></td>
															</tr>
															<!--<tr><td></td></tr>
															<tr>
														...
														<?php } ?>
														<p>消化単価＝（コース金額-値引）/コース回数</p>
														<p>消化金額＝消化単価✕消化回数</p>
														<p>残金＝消化金額-支払済金額</p>
														<p>解約手数料＝（コース金額-値引-消化金額）✕10%　（*マックス：2万円）</p>
														<p>返金額（手数料含）＝残金-既払金+解約手数料+ローン分割手数料＋ローンキャンセル料</p>
													</div>
											...
											<tr>
												...
												<td><input type="text" name="" value="<?php echo $sales['id'] ? ($sales['price']- $sales['balance']) : $data['balance'] ;?>" class="inp-form" disabled /></td>
											...
											<tr>
												...
												<td><input type="text" name="charge2" value="<?php echo ($sales['charge2']) ;?>" class="inp-form"  onChange="keisan2()" /></td>
											</tr>
											<tr>
												<th valign="top">ローンキャンセル手数料:</th>
												<td><input type="text" name="charge3" value="<?php echo ($sales['charge3'])  ;?>" class="inp-form"  onChange="keisan2()" /></td>
											</tr>
