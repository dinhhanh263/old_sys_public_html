...
if ( $dRtn3) {
	...
	while ( $data = $dRtn3->fetch_assoc() ) {
		...
		echo 	'<td>'.($course_type[$data['course_id']] ? $gContractStatus3[$data['status']] : ($data['status']==4 && $data['conversion_flg'] ? "プラン組替" : $gContractStatus7[$data['status']])).'</td>';
		echo 	'<td>'.$shop_list[$data['shop_id']].'</td>';
		echo 	'<td>'.$data['contract_date'].'</td>';
		echo 	'<td>'.$data['no'].'</td>';
		echo 	'<td title="'.$data['name'].'">'.($data['name_kana'] ? $data['name_kana'] : ($data['name'] ? $data['name'] : '無名')).'</td>';
