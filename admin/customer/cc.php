	...
	<div id="page-heading">
		...
		<h1>
			...
			<span style="margin-left:20px;">
				<a href="./?reg_date2=<?php echo $pre_date?>&shop_id=<?php echo $_POST['shop_id'];?>&ctype=<?php echo $_POST['ctype'];?>&rebook_flg=<?php echo $_POST['rebook_flg'];?>&adcode=<?php echo $_POST['adcode'];?>&line_max=<?php echo $_POST['line_max'];?>"><img src="../images/table/paging_left.gif" title="前日" /></a>
				...
				<a href="./?reg_date2=<?php echo $next_date?>&shop_id=<?php echo $_POST['shop_id'];?>&ctype=<?php echo $_POST['ctype'];?>&rebook_flg=<?php echo $_POST['rebook_flg'];?>&adcode=<?php echo $_POST['adcode'];?>&line_max=<?php echo $_POST['line_max'];?>"><img src="../images/table/paging_right.gif" title="翌日" /></a>
...
if ( $dRtn3->num_rows >= 1 ) {
	...
	while ( $data = $dRtn3->fetch_assoc() ) {
		...
		if($rsv['type']<>1)continue; //カウンセリング予約時のみ、
		...
		if($rsv['id']==$_POST['id'])$con_status .=";font-weight:bold;";

		$contract = Get_Table_Row("contract"," WHERE del_flg=0 and customer_id = '".addslashes($data['id'])."' and  (end_date>='".date("Y-m-d")."' or end_date>='0000-00-00') order by status in (0,7) DESC, contract_date desc,id desc");//最新予約
	...
	if($authority_level<=6 || ($authority['id']==106 || $authority['id']==1449) && $_POST['ctype']<2){
		echo 	'<td>'.($data['mobile'] ? $data['mobile'] : $data['tel']).'</td>';
		...
		echo 		'<a href="../reservation/edit.php?reservation_id='.$rsv['id'].'&from_cc=1" title="予約詳細" class="icon-1 info-tooltip"></a>';
