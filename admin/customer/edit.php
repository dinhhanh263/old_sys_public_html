								...
								<td>
									...
									<form action="" method="post" name="form1" enctype="multipart/form-data" onSubmit="return conf1('');">
										...
										<?php }?>
											<tr>
												...
												<td class="noheight">
													<table border="0" cellpadding="0" cellspacing="0">
														<tr valign="top">
															<td><input class="inp-form"  name="birthday" type="text" id="day2" value="<?php echo $data['birthday'];?>" /></td>
														</tr>
													</table>
												</td>
												<td></td>
											...
											<tr>
												<th valign="top">要注意箇所:</th>
												<td><select name="caution" class="styledselect_form_1"><?php Reset_Select_Key( $gCaution , $data['caution']);?></select></td>
											</tr>
											<tr>
												<th valign="top"></th>
												<td>場所：<input  class="inp-form" name="caution_place" type="text" value="<?php echo $data['caution_place'];?>" /></td>
												<td>　大きさ：約<input  class="inp-form" name="caution_size" type="text" value="<?php echo $data['caution_size'];?>" />cm</td>
											</tr>
											<tr>
											...
											<!-- <tr>
												...
												<td><select name="loan_delay_flg" class="styledselect_form_1"><?php Reset_Select_Key( $gLoanNG1 , $data['loan_delay_flg']);?></select></td>
											</tr> -->
											<tr>
										...
										<?php if($authority_level<=6 || ($authority['id']==106 || $authority['id']==1449) && $data['ctype']<2){?>
											<tr>
										...
										<?php }?>
											<tr>
												<th valign="top">接触不可:</th>
												<td><select name="rejected_flg" class="styledselect_form_1"><?php Reset_Select_Key( $gRejectNotification , $data['rejected_flg']);?></select></td>
											</tr>
											<tr>
												<th valign="top">登録店舗:</th>
												<td><?php echo $shop_list[$data['shop_id']] ?></td>
											...
											<tr class="gray">
												...
												<td><input  class="inp-form" name="introducer" type="text" value="<?php echo $data['introducer'];?>" /><span class="f11">※友達紹介は登録しない</span></td>
											</tr>
											<tr class="gray">
										...
										<?php  } ?>
											<tr>
												<th valign="top" class="li15">友達紹介（紹介元）:<span class="f11">※会員番号</span></th>
												<td>
													<input  class="inp-form" name="inviteCode" type="text" value="<?php echo isset($data_introducer_customer['no'])? $data_introducer_customer['no'] : '' ;?>" maxlength="11" />
													<style> .ajaxMessage { margin-top: 10px !important; } .formError { color:red; } </style>
											...
											<tr>
												<th valign="top">メールステータス:</th>
												<td><select name="mail_status" class="styledselect_form_1"><?php Reset_Select_Key( $mail_status , $data['mail_status']);?></select></td>
											</tr>
											<tr>
												<th valign="top">糖質リスク:</th>
												<td><select name="sugar_risk_id" class="styledselect_form_1"><?php Reset_Select_Key( $gRiskType , $data['sugar_risk_id']);?></select></td>
											</tr>
											<tr>
												<th valign="top">脂質リスク:</th>
												<td><select name="fat_risk_id" class="styledselect_form_1"><?php Reset_Select_Key( $gRiskType , $data['fat_risk_id']);?></select></td>
											</tr>
											<tr>
												<th valign="top">たんぱく質リスク:</th>
												<td><select name="protein_risk_id" class="styledselect_form_1"><?php Reset_Select_Key( $gRiskType , $data['protein_risk_id']);?></select></td>
											</tr>
											<tr>
												<th valign="top">遺伝子検査結果登録日:</th>
												<td class="noheight">
													<table border="0" cellpadding="0" cellspacing="0">
														<tr valign="top">
															<td><input class="inp-form"  name="gene_type_reg_date" type="text" id="day3" value="<?php echo $data['gene_type_reg_date']=="0000-00-00" ? "" : $data['gene_type_reg_date'] ;?>" /></td>
														</tr>
													</table>
												</td>
												<td></td>
...
<script>
	...
	$(function() {
		if ($('input:text[name=inviteCode]').val() !== "") {
			var inviteCode = $('input:text[name=inviteCode]').val();
			validInviteCode(inviteCode);
		}
	});

	$('input:text[name=inviteCode]').keypress(function(e) {
	...
	$('input:text[name=inviteCode]').keyup(function(e) {
	...
	$('input:text[name=inviteCode]').bind({
		cut:
			function() {
				$('.ajaxMessage').html('');
			},
	...
	function validInviteCode( inviteCode ) {
		if ( inviteCode !== void 0 && inviteCode !== "") {

				$.ajax({
					type: "POST",
					url: "../library/ajax/check_invite.php",
					data: {
					...
					success: function (json) {
						...
						if ( json !== null) {
							...
							if ( json.match(/^(.+?)[\s　]+(.+)$/) ) {
								$('.ajaxMessage').removeClass("formError");
								$('.ajaxMessage').html(json);
							} else {
								$('.ajaxMessage').addClass("formError");
								$('.ajaxMessage').html(json);
							}
