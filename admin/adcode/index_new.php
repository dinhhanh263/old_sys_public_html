<?php include_once('../library/adcode/index_new.php');?>
	...
	<div id="page-heading">
		<h1>広告コード別集計
			<span style="margin-left:20px;">
				<a href="./index_new.php?mode=display&access_date=<?php echo $pre_date?>&access_date2=<?php echo $pre_date2?>&agent_id=<?php echo $_POST['agent_id'];?>"><img src="../images/table/paging_left.gif" title="前日" /></a>
				...
				<a href="./index_new.php?mode=display&access_date=<?php echo $next_date?>&access_date2=<?php echo $next_date2?>&agent_id=<?php echo $_POST['agent_id'];?>"><img src="../images/table/paging_right.gif" title="翌日" /></a>
		...
		<div id="content-table-inner">
			...
			<div id="table-content">
				...
				<tr>
					...
					<th class="table-header-repeat line-left"><a href="">&nbsp;顧客CV<br>&nbsp;(total)</a></th>
					<!-- <th class="table-header-repeat line-left"><a href="">CVR</a></th> -->
					<th class="table-header-repeat line-left"><a href="">&nbsp;予約CV<br>&nbsp;(total)</a></th>
				</tr>
<?php
	if ( $data ) {
	foreach ( $data as $key=>$val ) {
	$adcode = Get_Table_Row("adcode"," WHERE job_flg=0 and id ='" . $key . "'");
	$agent_name = Get_Table_Col("agent","name"," WHERE id ='" . $adcode['agent_id'] . "'");

	  if(!$val['reg_all'] && !$val['reserv_reg_all']) continue;
?>
					<tr <?php echo($val['del_flg'] ?  'bgcolor="#CCCCCC"' : ($i%2==0 ? '' : ' class="alternate-row"'))?>>
						<td><?php echo($adcode['adcode']); ?></td>
						<td><?php echo ($adcode['name']); ?></td>
						<td style="width:140px;"><?php echo($agent_name); ?></td>
						<td style="width:60px;text-align:right;padding-right:5px;"><?php echo( $alarm_tab_begin . $val['reg_all'] .$cost_cnt. $alarm_tab_end ); ?></td>
						<td style="width:60px;text-align:right;padding-right:5px;"><?php echo( $alarm_tab_begin . $val['reserv_reg_all'] .$cost_cnt. $alarm_tab_end ); ?></td>
					</tr>
<?php
	$i++;
  }
...
?>
					<tr class="bgb" id="eee">
						...
						<td style="text-align:right;padding-right:5px;"><?php echo number_format($data['reserv_total_cv_cnt'] + 0); ?></td>
					</tr>
