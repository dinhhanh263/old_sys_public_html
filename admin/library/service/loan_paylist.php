...
// 検索条件の設定------------------------------------------------------------------------
$dWhere = " WHERE l.del_flg=0 AND l.loan_contract_no<>'' ";
if( $_POST['keyword'] != "" ){
	$dWhere .= " AND ( ";
...
$dWhere .= " AND SUBSTRING(l.loan_contract_date,1,7)<='".$ym2."'";
...
// データの取得------------------------------------------------------------------------
$dSql = "SELECT count(l.id) FROM " . $table . " l LEFT JOIN customer c ON c.id=l.customer_id AND l.customer_id<>0 AND c.del_flg=0".$dWhere;
$dRtn2 = $GLOBALS['mysqldb']->query( $dSql ) or die('query error'.$GLOBALS['mysqldb']->error);
$dGet_Cnt = $dRtn2->fetch_row()[0];

$dSql = "SELECT l.*,c.no,t.times,t.price,t.payment_loan,t.balance
		 FROM " . $table . " l
