...
if( $_POST['action'] == "edit" ) {
  ...
  if($authority_level>0 && $_POST['cancel_date']<$editable_date){
	...
	if($contract['status']==0 || $contract['status']==7) $GLOBALS['mysqldb']->query("update reservation set type=3 where type=2 and del_flg=0 and contract_id=".$contract['id']." and customer_id=".$_POST['customer_id']." and hope_date>'".$_POST['cancel_date']."'") or die('query error'.$GLOBALS['mysqldb']->error);

	// 売上tableに反映------------------------------------------------------------------------
	// 自動解約
	$_POST['type'] = 12;
	$_POST['contract_id'] = $contract['id'];
	$sales_field  = array("contract_id","type","customer_id","shop_id","staff_id","rstaff_id","course_id","fixed_price","discount","price","payment","payment_cash","payment_card","payment_transfer","payment_loan","charge","option_name","option_price","option_card","option_transfer","balance","memo","pay_date","cancel_date","reg_date","edit_date","terminate_day");

	$sales_field2 = array("contract_id","type","customer_id","shop_id","staff_id","rstaff_id","course_id","fixed_price","discount","price","payment","payment_cash","payment_card","payment_transfer","payment_loan","charge","balance","option_name","option_price","option_card","option_transfer","memo","pay_date","cancel_date","edit_date","terminate_day");
