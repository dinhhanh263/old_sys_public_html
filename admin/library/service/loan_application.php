...
if(date('n')<=10){
	for($month=date('n')+1;$month<=date('n')+1;$month++ ){
...
if( $_POST['action'] == "edit" ) {
	...
	if( empty( $gMsg  ) ){
		...
		elseif(!Get_Table_Col($table,"id"," WHERE del_flg=0 AND customer_id = '".addslashes($contract['customer_id'])."' AND contract_id = '".addslashes($_REQUEST['contract_id'])."'")){
			...
			// 受付番号付与
			// $_POST['recept_no'] = Get_Table_Col("loan_info","max(recept_no+1)"," WHERE id<>0");
			// array_push($common_field, 'recept_no');
		...
		if( $_POST['id'] ) {
			//$gMsg = '（登録完了）';
			//$complete_flg = 1;
			header('Location: ./loan_application_complete.php?loan_info_id='.$_POST['id']);
			exit;
		}
	}
}
...
if($_REQUEST['contract_id']){
    ...
    if ($_REQUEST['complete'] == 1) {
        $_POST['apl_flg'] = 1;
        Update_Data($table, ['apl_flg'], $data['id']);
    }
}
else $data = array();
...
// 頭金
$initial_payment = $data['initial_payment'] ? $data['initial_payment'] : ($contract['price']-$contract['payment_loan']);
// 申込金額
$payment_loan = $data['amount'] ? $data['amount'] : $contract['payment_loan'];
// 支払総額
$total_amount = $data['initial_payment'] ? ($data['initial_payment']+$data['amount_of_installments']) : ($contract['price']+$data['total_installment_commission']);
...
function validate(){
	...
	if( $_POST['initial_payment']<0 )			$gMsg .= "<br />※頭金が正しくありません。";

	//if( $_POST['amount']>350000 )				$gMsg .= "<br />※申込金額が正しくありません。";
   	// if( $_POST['amount']>350000 || $_POST['amount']<50000 )             $gMsg .= "<br />※申込金額が正しくありません。";

	if( $_POST['amount']>560000 )				$gMsg .= "<br />※申込金額が正しくありません。";
	//if( empty($_POST['number_of_payments']) )	$gMsg .= "<br />※支払回数を選択してください。";
	//else {
	//	if( $_POST['amount'] == 30000 && $_POST['number_of_payments'] > 10 ) $gMsg .= "<br />※10回までしか選択できません。";
	//	if( $_POST['amount'] == 40000 && $_POST['number_of_payments'] > 12 ) $gMsg .= "<br />※12回までしか選択できません。";
	//	if( $_POST['amount'] == 50000 && $_POST['number_of_payments'] > 18 ) $gMsg .= "<br />※18回までしか選択できません。";
	...
	//	if( $_POST['amount'] == 80000 && $_POST['number_of_payments'] > 24 ) $gMsg .= "<br />※24回までしか選択できません。";
	//}
    	...
    	if( empty($_POST['number_of_payments']) ) $gMsg .= "<br />※支払回数を選択してください。";
            ...
            if( $_POST['amount'] == 50000 && $_POST['number_of_payments'] > 10 ) $gMsg .= "<br />※10回までしか選択できません。";
            if( $_POST['amount'] == 60000 && $_POST['number_of_payments'] > 12 ) $gMsg .= "<br />※12回までしか選択できません。";
            if( $_POST['amount'] == 70000 && $_POST['number_of_payments'] > 12 ) $gMsg .= "<br />※12回までしか選択できません。";
            if( $_POST['amount'] == 80000 && $_POST['number_of_payments'] > 18 ) $gMsg .= "<br />※18回までしか選択できません。";
            if( $_POST['amount'] == 90000 && $_POST['number_of_payments'] > 18 ) $gMsg .= "<br />※18回までしか選択できません。";
            if( $_POST['amount'] == 100000 && $_POST['number_of_payments'] > 18 ) $gMsg .= "<br />※18回までしか選択できません。";
            if( $_POST['amount'] == 110000 && $_POST['number_of_payments'] > 24 ) $gMsg .= "<br />※24回までしか選択できません。";
            if( $_POST['amount'] == 120000 && $_POST['number_of_payments'] > 24 ) $gMsg .= "<br />※24回までしか選択できません。";
            if( $_POST['amount'] == 130000 && $_POST['number_of_payments'] > 30 ) $gMsg .= "<br />※30回までしか選択できません。";
            if( $_POST['amount'] == 140000 && $_POST['number_of_payments'] > 30 ) $gMsg .= "<br />※30回までしか選択できません。";
        }
	...
	if( empty($_POST['living_grant']) )			$gMsg .= "<br />※生活費を選択してください。";
	...
	elseif($_POST['annual_income']>9999)		$gMsg .= "<br />※年収が正しくありません。";
	...
	if( $_POST['identification_type'] == 2){
		if(empty($_POST['identification_number']) )	$gMsg .= "<br />※パスポート番号を入力してください。";

		elseif(!preg_match("/^[A-Z]{2}[0-9]{7}$/", $_POST['identification_number'])) $gMsg .= "<br />※パスポート番号は半角大文字英字２桁+半角数字７桁です。";
	}

	if( $_POST['identification_type'] == 4){
		if(empty($_POST['identification_number']) )	$gMsg .= "<br />※在留カード番号を入力してください。";

		elseif(!preg_match("/^[A-Z]{2}[0-9]{8}[A-Z]{2}$/", $_POST['identification_number'])) $gMsg .= "<br />※在留カード番号は半角大文字英字２桁+半角数字８桁+半角大文字英字２桁です。";
	}

	if( empty($_POST['call_timezone']) )		$gMsg .= "<br />※電話連絡可能時間帯を選択してください。";

	if( empty($_POST['credit_app_agree']) )		$gMsg .= "<br />※「クレジットお申し込みの内容同意」にチェックを入れてください。";

	if( empty($_POST['privacy_agree']) )		$gMsg .= "<br />※「個人情報の取り扱いに関する同意」にチェックを入れてください。";

	if( empty($_POST['info_change_agree']) )	$gMsg .= "<br />※「顧客情報の変更に関する同意」にチェックを入れてください。";
