	...
	if(file_exists($import_file) && !$gMsg){
		...
		*/
		...
		foreach($lines as $val){
			$val = trim($val);
			list($option_date,$trade,$no,$name,$payment,$card_name,$card_no) = explode(",",$val);
			...
			// 条件：振込済
			if( $trade=="成功" && $pay_date<>"" && strstr($pay_date, "-") && $payment){
				$result_sql = "insert pay_monthly set title='".$_POST['title']."',shop_id='".$_POST['shop_id']."',pay_type=2,pay_date='".$pay_date."',option_date='".$option_date."',reg_date='".$import_date."',no='".$no."', name='".$name."', card_name='".$card_name."', card_no='".$card_no."', payment='".$payment."', result=0";
				
				// 条件：会員番号が部分一致、かつ、名前またカナが部分一致,カード名義があればカード名義から検索
				// if( $customer_array = Get_Table_Array("customer","id"," WHERE del_flg=0 AND no = '".$no."' AND  (replace(card_name,'　',' ')='".$card_name."' OR replace(card_name,'　',' ')='".$card_name2."') AND card_name<>'' AND card_no='".$card_no."' AND card_no<>'' " ) ){
				if( $customer_array = Get_Table_Array("customer","id"," WHERE del_flg=0 AND no = '".$no."' AND  (replace(card_name,'　',' ')='".$card_name."' OR replace(card_name,'　',' ')='".$card_name2."') AND card_name<>'' " ) ){
			...
			}elseif( $trade=="失敗" && $payment){
				$result_sql = "insert pay_monthly set title='".$_POST['title']."',shop_id='".$_POST['shop_id']."',pay_type=2,pay_date='".$pay_date."',reg_date='".$import_date."',no='".$no."', name='".$name."', card_name='".$card_name."', card_no='".$card_no."', payment='".$payment."', result=0";
				// if( $customer_array = Get_Table_Array("customer","id"," WHERE del_flg=0 AND no = '".$no."' AND  (replace(card_name,'　',' ')='".$card_name."' OR replace(card_name,'　',' ')='".$card_name2."')  AND card_name<>'' AND card_no='".$card_no."' AND card_no<>'' " ) ){
				if( $customer_array = Get_Table_Array("customer","id"," WHERE del_flg=0 AND no = '".$no."' AND  (replace(card_name,'　',' ')='".$card_name."' OR replace(card_name,'　',' ')='".$card_name2."')  AND card_name<>'' " ) ){
