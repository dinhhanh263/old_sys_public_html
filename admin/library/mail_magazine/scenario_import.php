...
if( $_POST['mode'] == "csv_import" ){
	...
	if(file_exists($import_file)&& !$gMsg){
		...
		// データ名新規
		$GLOBALS['mysqldb']->query("insert mail_scenario_info set name = '$data_name', date='$import_date', type='{$_POST['type']}', genre='{$_POST['genre']}'") or die('query error'.$GLOBALS['mysqldb']->error);
		...
		if($total){
			$GLOBALS['mysqldb']->query("update mail_scenario_info set total='$total' where id='$data_name_id'") or die('query error'.$GLOBALS['mysqldb']->error);
		}else{
			// 処理件数がなければ、データ名を削除
