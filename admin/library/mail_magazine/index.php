...
if($_POST['scenario_id']) $dWhere .= " and scenario_id=".$_POST['scenario_id'];
...
if($_POST['mode']=="test" && $_POST['test_mail']){
	...
	if($_POST['send_now']){
		...
		while ( $data = $dRtn->fetch_assoc() ) {
			...
			if( $_POST['format'] && sendMailHtml( $data['mail'], $_POST['subject'], $body2, $_POST['sender'], $cc="", $bcc=$_POST['sender'] , MAIL_RETURN ) ) {
				$send_flg = true;$cnt +=1;
			}
			
			elseif( sendMail( $data['mail'], $_POST['subject'], $body2, $_POST['sender'], $cc="", $bcc=$_POST['sender'] , MAIL_RETURN ) ) {
				$send_flg = true;$cnt +=1;
			}
			sleep(10); // 10秒まち
		}
