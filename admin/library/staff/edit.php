...
if( $_POST['action'] == "edit" ) {
	...
	if( $data_ID ) {
		...
		if($staff['auth_id']){
			//アカウント更新
			$GLOBALS['mysqldb']->query("update staff set auth_id='".$staff_auth['id']."',edit_date=NOW() where id='".$data_ID."'") or die('query error'.$GLOBALS['mysqldb']->error);
			$GLOBALS['mysqldb']->query("update authority set staff_id='".$data_ID."',authority='".$staff['type']."',login_id='".$staff['login_id']."',password='".$staff['password']."',edit_date=NOW() where id='".$staff['auth_id']."'") or die('query error'.$GLOBALS['mysqldb']->error);
		}else{
			//アカウント新規
			$GLOBALS['mysqldb']->query("insert authority set staff_id='".$data_ID."',authority='".$staff['type']."',login_id='".$staff['login_id']."',password='".$staff['password']."',reg_date=NOW(), edit_date=NOW()") or die('query error'.$GLOBALS['mysqldb']->error);
			$staff_auth = Get_Table_Row("authority"," WHERE staff_id = '".$data_ID."'");
			$GLOBALS['mysqldb']->query("update staff set auth_id='".$staff_auth['id']."',reg_date=NOW(),edit_date=NOW() where id='".$data_ID."'") or die('query error'.$GLOBALS['mysqldb']->error);
		}
