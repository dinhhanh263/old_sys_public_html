...
// 引き落とし情報登録処理------------------------------------------------------------------------
if( $_POST['mode']=='reg' && empty($_POST['pay_shop']) ){
			$pay_shop_error_msg = "<br><span style='color:red;'>※ 登録店舗が未選択でした。</span>";
}
elseif( $_POST['mode']=='reg' && empty($_POST['pay_reg_date']) ){
			$pay_reg_date_error_msg = "<br><span style='color:red;'>※ 登録日付が未入力です。</span>";
}
elseif( !empty($_POST['pay_reg_date']) && (!strptime($_POST['pay_reg_date'], '%Y-%m-%d') || $_POST['pay_reg_date'] > date('Y-m-d'))){
			$pay_reg_date_error_msg = "<br><span style='color:red;'>※ 登録日付が正しくありません。</span>";
}
elseif( $_POST['mode']=='reg' && empty($_POST['pay_type']) ){
			$pay_pay_type_error_msg = "<br><span style='color:red;'>※ 支払方法が未選択です。</span>";
}
elseif( $_POST['contract_id'] && $_POST['mode'] == "reg" && $_POST['id'] >= 1 ){
	$sql = "UPDATE contract SET pay_shop='".$_POST['pay_shop']."' ,pay_reg_date='".$_POST['pay_reg_date']."' , payinfo_del_flg=0,pay_type='".$_POST['pay_type']."' , card_type='".$_POST['card_type']."'";
  ...
  if($_POST['pay_type']==2 ){
	...
	if($_POST['card_limit_year'] <>"" && $_POST['card_limit_month'] <>""){
		...
		if($card_error_flg ==false && is_numeric($_POST['card_limit_year']) && is_numeric($_POST['card_limit_month'])){
			...
			$in_end_ymd    = date("Y/m/t"); // 今月末日
			
			if(checkLimitDate($target_ymd,$in_end_ymd)==true){
		...
		if(($card_error_flg ===false && $_POST['card_limit_year']<>"" && $_POST['card_limit_month']<>"") ){
		...
		} else if($_POST['card_limit_year']=="" || $_POST['card_limit_month']=="") {
...
// TODO:前回がパック⇒新月額だったときはパックのお金で2ヵ月分充当するケースがあるため、下記の処理は使えない。////////////////////
// 新月額で旧契約IDがある方(プラン変更者対応)　2017/03/07 correct by shimada
...
// 	$cSql3 .= " WHERE t.course_id = u.id AND u.type =1 AND t.del_flg=0 AND t.customer_id=".$_POST['customer_id'];
// 	$cSql3_result = Get_Result_Sql_Row($cSql3);
// 	// 月額契約が複数あったとき、二度目以上となる
// 	if($cSql3_result['MIN_T_ID'] <> $cSql3_result['MAX_T_ID']){
// 		if($contract['id'] == $cSql3_result['MAX_T_ID']){
...
if($cSql2_result <>""){
	...
	//////// 旧記述　真ん中のelseifのとき、相殺したものすべて未払いとしてしまっているため条件の追加が必要（過払いのこともある）
	//////// しばらく運用して問題なければ下記のコメントアウトをけしてください。　2017/02/22 add by shimada
