...
if($_POST['tel_status']==1)$dWhere .= " AND  r.today_status <>1";
elseif($_POST['tel_status']==2)$dWhere .= "";
else $dWhere .= " AND  r.today_status = 0";

// データの取得------------------------------------------------------------------------
// WEBのみ、登録日：当日、予約日：二日後

$dSql = 'SELECT r.hope_date , r.hope_time,r.reg_date , c.no , c.name_kana ,  c.tel, c.pair_name_kana , c.pair_tel , s.name,
s.id , r.id as reservation_id, c.id , r.today_status ,r.today_cnt,r.memo4 ,r.memo3
FROM reservation AS r, customer AS c, shop AS s
WHERE r.del_flg = 0
AND r.type = 1
AND r.route = 1';
if($_POST['rsv_today']){
	$dSql .= ' AND r.reg_date > "'. date("Y-m-d 21:00:00",strtotime("-1 day")) .'" AND r.hope_date = "'. date("Y-m-d") .'"';
}else{
	$dSql .= ' AND r.hope_date =  (SELECT date_format((SELECT adddate(now(), interval + 3 day)),"%Y-%m-%d"))';
}
...
AND r.shop_id = s.id
'.$dWhere.'
ORDER BY r.hope_date,r.hope_time';
$dRtn3 = $pdo->query( $dSql ) or kireimo_die('query error'.mysql_error());
