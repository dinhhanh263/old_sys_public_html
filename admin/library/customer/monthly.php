...
if( $_POST['course']==1 ) $dWhere .= " AND c.course_id in (".$old_month_id." )"; // 旧月額
if( $_POST['course']==3 ) $dWhere .= " AND c.course_id in (".$new_month_id." )"; // 新月額

// ～2016/12/07以前個別で出していた月額コース
...
// 2016/12/07～コース区分で月額のコースを取得
$from = "FROM contract AS c,course AS u,customer AS cs WHERE c.course_id=u.id AND c.customer_id=cs.id AND c.del_flg =0 AND cs.del_flg =0 AND c.status =0 AND u.type=1 ";
