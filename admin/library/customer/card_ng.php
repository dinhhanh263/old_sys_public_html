...
if( $_REQUEST['debit_type'] && $_REQUEST['card_ng']) {
	if( $_REQUEST['debit_type']==1) $from .= " AND cs.digicat_ng_flg=".$_POST['card_ng'];
	if( $_REQUEST['debit_type']==2) $from .= " AND cs.nextpay_end_ng_flg=".$_POST['card_ng'];
	if( $_REQUEST['debit_type']==3) $from .= " AND cs.nextpay_op_ng_flg=".$_POST['card_ng'];
}elseif($_REQUEST['debit_type']){
	if( $_REQUEST['debit_type']==1) $from .= " AND cs.digicat_ng_flg<>0";
	if( $_REQUEST['debit_type']==2) $from .= " AND cs.nextpay_end_ng_flg<>0";
	if( $_REQUEST['debit_type']==3) $from .= " AND cs.nextpay_op_ng_flg<>0";
}elseif($_REQUEST['card_ng']){
	 $from .= " AND (cs.digicat_ng_flg=".$_POST['card_ng']." || cs.nextpay_end_ng_flg=".$_POST['card_ng']." || cs.nextpay_op_ng_flg=".$_POST['card_ng']." )";
}else{
	$from .= " AND (cs.digicat_ng_flg<>0 || cs.nextpay_end_ng_flg<>0 || cs.nextpay_op_ng_flg<>0 )";
}
...
$dSql = "SELECT c.*,cs.no as no,cs.name as name,cs.name_kana as name_kana,cs.tel as tel,cs.digicat_ng_flg, cs.nextpay_end_ng_flg,cs.nextpay_op_ng_flg,cs.reg_date as reg_date ".$from.$dWhere." order by c.contract_date LIMIT ".$dStart.",".$dLine_Max;
