...
// データの取得------------------------------------------------------------------------
// 【条件】消化あり、会員タイプ：テスト以外 2016/12/08 add by shimada（下記、会計士さん確認用DBができたら会員タイプを一般以外も表示させることを検討する。）
// $dSql = "SELECT s.*,c.no as no,c.ctype,c.name as name,c.name_kana as name_kana,c.introducer_type,r.length,r.part FROM " . $table . " s left join reservation r on s.reservation_id=r.id,customer c WHERE s.r_times>0 and s.customer_id=c.id AND c.del_flg=0 AND c.ctype<>5  AND s.del_flg = 0".$dWhere." ORDER BY s.".$order;
// 【条件】消化あり、会員タイプ：一般 2016/12/26 add by shimada
