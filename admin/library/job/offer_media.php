...
// if( $_POST['keyword'] != "" ){
	// $dWhere .= " AND ( ";
	// $dWhere .= "  replace(entry_name, '　', '') LIKE '%". str_replace("　","",mb_convert_kana($_POST['keyword'],"SKV", "UTF-8") )."%'";
	// $dWhere .= " or replace(entry_name_kana, '　', '') LIKE '%".str_replace("　","",mb_convert_kana($_POST['keyword'],"SKV", "UTF-8") )."%'";
	// $dWhere .= " or now_email LIKE '%".addslashes( $_POST['keyword'] )."%'";
	// $dWhere .= " or now_address_1 LIKE '%".addslashes( $_POST['keyword'] )."%'";
	// $dWhere .= " ) ";
// }
...
$dSql = "SELECT * FROM " . $table . " WHERE del_flg = 0".$dWhere." ORDER BY id ASC LIMIT ".$dStart.",".$dLine_Max;
...
//ADリスト
// $adcode_sql = $GLOBALS['mysqldb']->query( "select * from adcode where job_flg=1 order by name" ) or die('query error'.$GLOBALS['mysqldb']->error);
// $adcode_list[0] = "全媒体";
// while ( $result = $adcode_sql->fetch_assoc() ) {
// 	$adcode_list[$result['id']] = $result['name'];
// }
//求人媒体リスト
// $job_media_sql  = $GLOBALS['mysqldb']->query( "select * from job_media WHERE del_flg = 0 AND status=2 ORDER BY id" ) or die('query error'.$GLOBALS['mysqldb']->error);
